...
(require (only-in rosette union? union-contents union expression)
         (only-in rosette/base/core/polymorphic ite ite*)
         ...
         (only-in rosette/base/core/bitvector @bvadd @bveq)

         (only-in rosette/base/core/symfix get-mutator+field))
       ...
       (cond
         ...
         [else (let ([x (car xs)])
                 (cons (match x
                         ...
                         [(expression (== @!)
                                      (expression (== @bveq)
                                                  (expression (== @bvadd) _ _)
                                                  (expression (== @bvadd) _ _)))
                          'serval_hack]
                         [(expression (or (== ite) (== ite*)) _ ...) 'union]
                         [(? get-mutator+field)                      'struct]
                         [(? term?)                                  'symbolic]
                         [_                                          'concrete])
                       (loop (cdr xs))))])))))
