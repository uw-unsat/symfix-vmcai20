...
(struct profiler-reporter (profile [metrics #:mutable] [finitizing #:mutable])
  ...
  (let ([next-id (let ([n -1]) (thunk (begin0 n (set! n (sub1 n)))))])
    (lambda (self . rest)
      (match rest
        [(list 'struct-compress-start struct-name)
         (define profile (profiler-reporter-profile self))
         (when profile
           (define metrics (get-current-metrics/call self))
           (define new (profile-event-enter-struct metrics (next-id) struct-name))
           (profile-state-append! profile new))]
        [(list 'struct-compress-finish)
         (define profile (profiler-reporter-profile self))
         (when profile
           (define metrics (get-current-metrics/call self))
           (define new (profile-event-exit-struct metrics))
           (profile-state-append! profile new))]
