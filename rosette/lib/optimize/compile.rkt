...
(require racket/function
         syntax/stx syntax/kerncase racket/keyword-transform
         rosette/lib/util/utility/main
         syntax/parse
         (only-in "record.rkt" record-apply! record-source!)
         (only-in "../syntax-utils/utils.rkt" make-rosette-compiler))
(provide make-symbolic-profile-compile-handler
         symbolic-profile-compile-handler
         symbolic-profile-rosette-only?)
...
(define (is-original*? stx)
  (and (syntax-line stx)
       (> (syntax-line stx) original-base-line)))
...
(define (make-symbolic-profile-compile-handler
         #:transformer [transform (λ xs (apply values xs))]
         #:dump [dump #f]
         #:load? [load? #f]
         #:instant? [instant? #f])
  (cond
    [dump (make-rosette-compiler transform
                                 #:dump dump
                                 #:instant? #t
                                 #:size-limit 50000)]
    [else (make-rosette-compiler
           (λ (stx phase) (values ((compose annotate-top transform) stx phase) phase))
           #:size-limit 50000
           #:instant? instant?
           #:before (λ (stx)
                      (cond
                        [load? stx]
                        [else (set-add! original-files (syntax-source stx))
                              (add-annotate-property stx)])))]))
