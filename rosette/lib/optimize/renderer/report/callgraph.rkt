...
(define (render-event? event)
  (or (profile-event-enter? event)
      (profile-event-enter-struct? event)
      (profile-event-exit? event)
      (profile-event-exit-struct? event)
...
(define (render-event event)
  (match event
    ...
    [(profile-event-enter-struct met id struct-type)
     (hash 'type "ENTER-STRUCT"
           'id id
           'struct struct-type
           'metrics (metrics->hash met))]
    [(profile-event-exit met out)
     (hash 'type "EXIT"
           ...
           'metrics (metrics->hash met))]
    [(profile-event-exit-struct met)
     (hash 'type "EXIT-STRUCT"
           'metrics (metrics->hash met))]
