    ...
    [(_ test-expr then-expr else-expr)
     (quasisyntax/loc stx
       (branch-and-merge test-expr
                         #,(quasisyntax/loc #'then-expr (thunk then-expr))
                         #,(quasisyntax/loc #'else-expr (thunk else-expr))))]))
...
(define-syntax (@when stx)
  (syntax-case stx ()
    [(_ test body ...) (quasisyntax/loc stx (@if test #,(quasisyntax/loc stx (let () body ...)) (void)))]))
