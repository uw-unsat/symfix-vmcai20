...
(require (for-syntax "../core/lift.rkt" racket/syntax)
         ...
         (only-in "../core/reporter.rkt" current-reporter))

(provide @make-struct-type
         @make-struct-field-accessor
         @make-struct-field-mutator
         (rename-out [struct-type-immutables @struct-type-immutables]
...
(struct struct-type (pred super native? make ref set! fields immutable? transparent? procedure? equal+hash compress-negate orig-name immutables)
     ...
     (cond
       [(or force?
            (and (struct-type-immutable? type)
                 (struct-type-transparent? type)
                 (not (struct-type-equal+hash type))
                 (not (struct-type-compress-negate type))))

        ;; send event
        ((current-reporter) 'struct-compress-start (struct-type-orig-name type))
        (call-with-exception-handler
         (lambda (e)
           ((current-reporter) 'struct-compress-finish)
           e)
         (thunk
          (define result (struct-compress type ps))
          ((current-reporter) 'struct-compress-finish)
          result))]
       [else ps]))
